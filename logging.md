...
With curl, you can include the `-u USERNAME` option to specify a username.  Or, to avoid re-entering the password repeatedly, use a `.netrc` file in the home directory that stores the user name and password and the `-n` option for curl:
